...
vprašanje več kot očiten \textendash\hspace{1pt} izdelala ga je oseba, ki so ji to delo naprtili. Naprtili je
...
\[
   \text{velikostTabele}  = \text{številoUrNaDa}n \cdot \text{številoDelovnihDni}
\]
...
je preprost \textendash\hspace{1pt} vsak urnik mora biti izdelan tako, da zadošča vsem strogim omejitvam.
...
omejitve so bile kršene ter kolikokrat se kršitev pojavi. S $k_n$
označimo število kršitev omejitve $n$, z $u_n$ pa njeno utež. Z naslednjo formulo izračunamo
kakovost urnika:
\[
...
Za lažjo ilustracijo bi lahko potegnili kar nekaj vzporednic med programiranjem in \textendash\hspace{1pt} ne
boste verjeli \textendash\hspace{1pt} kuhanjem. Za kuhanje potrebujemo recept, ki je sestavljen iz dveh delov.
...
Požrešna metoda je ena izmed strategij, s katerimi lahko rešujemo optimizacijske probleme \textendash\hspace{1pt}
...
% začetek okolja itemize
\begin{itemize}
   \item \textit{Vstajanje} srečanja v urnik.
...
% To popravite z ukazom `\noindent` ali pa tako, da odstranite prazno vrstico.
% (Nekoliko lepše izgleda, če v prazno vrstico vstavite % za komentar.)
\noindent \textit{Oklica} operacije je množica vseh možnih sprememb, ki jih lahko s to operacijo
...
Slabost do sedaj obravnavanih tehnik še vedno ostaja \textendash\hspace{1pt} ko enkrat obtičimo v lokalnem
...
Rešitev se ponuja kar sama \textendash\hspace{1pt} za naslednjih \textit{n} korakov prepovemo vsako spremembo, ki jo
...
Imamo torej seznam prepovedanih sprememb \textendash\hspace{1pt} \textit{tabujev} in na vsakem koraku za izbrano
...
uporabili katero izmed tehnik \textendash\hspace{1pt} mogoče bi katero izmed teh postopanj dalo celo boljši rezultat.
